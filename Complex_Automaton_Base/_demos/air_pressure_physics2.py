...
class FlowCell(CACell):
    ...
    def sense_neighborhood(self, neighborhood):
        for cell in neighborhood:
            if not cell.is_solid and not self.is_solid:
                d_pressure = self.pressure - cell.pressure
                flow = self.flow * d_pressure
                a = self.pressure / 8
                b = -cell.pressure / 8
                if a < b:
                    flow = numpy.clip(flow, a, b)
                else:
                    flow = numpy.clip(flow, b, a)
                self.new_pressure -= flow
                cell.new_pressure += flow

    def update(self):
        #pass
        self.pressure = self.new_pressure
        #self.new_pressure = self.pressure
...
class FlowIO(InputHandler):
    ...
    def custom_mouse_action(self, button):
        ...
        if button == 1:
            cell_x = int(self.mx)
            cell_y = int(self.my)
            self.sys.ca.ca_grid[cell_x, cell_y].turn = self.sys.ca.ca_grid[cell_x, cell_y].gc.TIME_STEP
            self.sys.ca.ca_grid[cell_x, cell_y].pressure = 100000.0
            #self.sys.ca.ca_grid[cell_x, cell_y].new_pressure = 100000.0
